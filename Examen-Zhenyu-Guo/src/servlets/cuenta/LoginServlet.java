	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session != null) {
			if ((session.getAttribute("login") != null)) { // L2
				response.sendRedirect(contexto.getContextPath() + "/MostrarProductos"); // L3
			}
		}
			...
			if (request.getMethod().equals("POST")) { // si venimos de enviar el formulario...
              ...
              // c. Error: no se encuentra el usuario en la base de datos
				  try {
					...
					String consulta = "SELECT * FROM cliente where login=\""+request.getParameter("login")+"\"";
              ...
              // 1. Crear un objeto de la clase Usuario con los datos obtenidos de la BD   
								Usuario user=new Usuario(rset.getString("login"), rset.getString("password"), rset.getString("nombre"), rset.getDouble("gasto"), rset.getString("direccion"));
